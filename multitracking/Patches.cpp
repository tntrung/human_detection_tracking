...
PatchesRegularScan::PatchesRegularScan(Rect imageROI, Rect validROI, Size patchSize,Rect newrect[],int sovungrect, float relOverlap)
{
	calculatePatches (imageROI, validROI, patchSize,newrect,sovungrect, relOverlap);
}
...
void PatchesRegularScan::calculatePatches(Rect imageROI, Rect validROI, Size patchSize, float relOverlap)
{
	...
	/*
	//khu loi
	if(ROI.width<patchSize.width)
		ROI.width=patchSize.width;
	if(ROI.height<patchSize.height)
		ROI.height=patchSize.height;
	*/
	int stepCol = (int)floor((1-relOverlap) * patchSize.width+0.5);
	int stepRow = (int)floor((1-relOverlap) * patchSize.height+0.5);
...
bool mxf_thuocvungsearch(const Rect &a,const Rect &b)
{
	if(b.upper<a.upper || b.left<a.left || b.height+b.upper>a.height+a.upper || b.width+b.left>a.width+a.left)
		return false;
	return true;
}
Rect mxf_resizerect(Rect rect,Size size)
{
	rect.left=rect.left+rect.width/2-size.width/2;
	rect.upper=rect.upper+rect.height/2-size.height/2;
	rect.width=size.width;
	rect.height=size.height;
	return rect;
}
void PatchesRegularScan::calculatePatches(Rect imageROI, Rect validROI, Size patchSize,Rect newrect[],int sovungrect, float relOverlap)
{
	if ((validROI == imageROI))
		ROI = imageROI;
	else
	...
	if(ROI.height<patchSize.height)
		ROI.height=patchSize.height;
	*/
	//lay so vung rect thuoc hinh anh
	Rect *vungrect=new Rect[sovungrect];	
	int sorecthople=0;
	for(int i=0;i<sovungrect;i++)
	{
		newrect[i]=mxf_resizerect(newrect[i],patchSize);
		if(mxf_thuocvungsearch(imageROI,newrect[i]))
			vungrect[sorecthople++]=newrect[i];
		else
			vungrect[sorecthople++]=Rect(-1,-1,-1,-1);
	}
	int stepCol = (int)floor((1-relOverlap) * patchSize.width+0.5);
	int stepRow = (int)floor((1-relOverlap) * patchSize.height+0.5);
	if (stepCol <= 0) stepCol = 1;
	...
	numPatchesX=0; numPatchesY=0;
	for(int i=0;i<sorecthople;i++)
	{
		numPatchesY++;
		//numPatchesX++;
		patches[curPatch].width = vungrect[i].width;//patchSize.width;
		patches[curPatch].height = vungrect[i].height;//patchSize.height;
		patches[curPatch].upper = vungrect[i].upper;//.ucurRow+ROI.upper;
		patches[curPatch].left = vungrect[i].left;//curCol+ROI.left;
		curPatch++;
	}
	/*
	for (int curRow=0; curRow< ROI.height-patchSize.height+1; curRow+=stepRow)
...
void PatchesRegularScaleScan::calculatePatches(Rect imageROI, Rect validROI, Size patchSize, float relOverlap, float scaleStart, float scaleEnd, float scaleFactor)
{
	...
	{   
		...
		stepCol = (int)floor((1-relOverlap) * curPatchSize.width+0.5);
		stepRow = (int)floor((1-relOverlap) * curPatchSize.height+0.5);
		if (stepCol <= 0) stepCol = 1;
		if (stepRow <= 0) stepRow = 1;
