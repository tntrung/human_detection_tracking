...
float StrongClassifier::eval(CvHistogram*hist, ImageRepresentation *image, Rect ROI)
{
	float value = 0.0f;
	...
	for (curBaseClassifier = 0; curBaseClassifier<numBaseClassifier; curBaseClassifier++)
		value+= baseClassifier[curBaseClassifier]->eval(hist, image, ROI)*alpha[curBaseClassifier];
	return value;
}
float StrongClassifier::eval(CvHistogram *image, Rect ROI)
{
	float value = 0.0f;
...
bool StrongClassifier::update(CvHistogram*hist, ImageRepresentation *image, Rect ROI, int target, float importance) 
{
	assert (true);
	return false;
}
bool StrongClassifier::update(CvHistogram *image, Rect ROI, int target, float importance) 
{
	assert (true);
	return false;
}
