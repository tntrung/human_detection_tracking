...
bool WeakClassifierHaarFeature::update(CvHistogram*hist,ImageRepresentation *image, Rect ROI, int target) 
{
	float value;
	
	bool valid = m_feature->eval (hist,image, ROI, &value); 
	if (!valid)
		return true;
...
int WeakClassifierHaarFeature::eval(CvHistogram*hist, ImageRepresentation *image, Rect ROI) 
{
	float value;
	bool valid = m_feature->eval(hist,image, ROI, &value); 
	if (!valid)
