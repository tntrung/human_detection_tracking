...
public:
	...
	virtual float eval(CvHistogram*hist, ImageRepresentation *image, Rect ROI); 
	virtual float eval(CvHistogram*image, Rect ROI); 

	virtual bool update(ImageRepresentation *image, Rect ROI, int target, float importance = 1.0f); 
	virtual bool update(CvHistogram*image, Rect ROI, int target, float importance = 1.0f); 
	virtual bool updateSemi(ImageRepresentation *image, Rect ROI, float priorConfidence);
	virtual bool update(CvHistogram*hist,ImageRepresentation *image, Rect ROI, int target, float importance = 1.0f); 
