...
BaseClassifier::BaseClassifier(int numWeakClassifier, int iterationInit, Size patchSize,const int feature)
{
	this->m_numWeakClassifier = numWeakClassifier;
	this->m_iterationInit = iterationInit;
	...
	m_idxOfNewWeakClassifier = numWeakClassifier;
	if(feature==0)//haar
		generateRandomClassifier_haar(patchSize);
	else if(feature==1)//histogram
		generateRandomClassifier_his(patchSize);
	else if(feature==2)//histogram (only)
		generateRandomClassifier_his(patchSize);
	else if(feature==3)//haar + histogram
		generateRandomClassifier_haar(patchSize);
...
void BaseClassifier::generateRandomClassifier_his(Size patchSize)
{
	for (int curWeakClassifier = 0; curWeakClassifier< m_numWeakClassifier+m_iterationInit; curWeakClassifier++)
	{
		weakClassifier[curWeakClassifier] = new WeakClassifierHistogramFeature(patchSize);
	}			
}

void BaseClassifier::generateRandomClassifier_haar(Size patchSize)
{
	for (int curWeakClassifier = 0; curWeakClassifier< m_numWeakClassifier+m_iterationInit; curWeakClassifier++)
	{
		weakClassifier[curWeakClassifier] = new WeakClassifierHaarFeature(patchSize);
...
int BaseClassifier::eval(CvHistogram *image, Rect ROI) 
{
	return weakClassifier[m_selectedClassifier]->eval(image, ROI);	

}
int BaseClassifier::eval(CvHistogram *hist,ImageRepresentation *image, Rect ROI) 
{
	return weakClassifier[m_selectedClassifier]->eval(hist,image, ROI);	
}
...
void BaseClassifier::trainClassifier(CvHistogram*hist, ImageRepresentation* image, Rect ROI, int target, float importance, bool* errorMask) 
{
	//get poisson value
	double A = 1;
	int K = 0;
	...
	for (int curK = 0; curK <= K; curK++)
		for (int curWeakClassifier = 0; curWeakClassifier < m_numWeakClassifier+m_iterationInit; curWeakClassifier++)
			errorMask[curWeakClassifier] = weakClassifier[curWeakClassifier]->update (hist,image, ROI, target); 
...
void BaseClassifier::trainClassifier(CvHistogram* image, Rect ROI, int target, float importance, bool* errorMask) 
{
	//get poisson value
	double A = 1;
	int K = 0;
