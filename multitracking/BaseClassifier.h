...
public:
	...
	BaseClassifier(int numWeakClassifier, int iterationInit, Size patchSize,const int features); 
	...
	void trainClassifier(CvHistogram*hist,ImageRepresentation* image, Rect ROI, int target, float importance, bool* errorMask); 
	
	void trainClassifier(CvHistogram* image, Rect ROI, int target, float importance, bool* errorMask); 
	...
	int eval(CvHistogram*hist, ImageRepresentation* image, Rect ROI); 
...
protected:
	...
	void generateRandomClassifier_his (Size patchSize);
	void generateRandomClassifier_haar(Size patchSize);
};
